...
def rbf(x_1, x_2, sigma=1.):
    ...
    distances = torch.sum((x_1[:, np.newaxis, :] - x_2[np.newaxis, :, :]) ** 2, axis = -1)
    distances = torch.exp(-0.5 * distances / sigma )### YOUR CODE HERE
    ...
    def fit(self, X, Y):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, n_obj, self.batch_size):
                ...
                epoch_loss += loss.item()   # Add the loss

            if self.verbose: print(f"Epoch: {epoch+1}, loss: {(epoch_loss / n_obj):.4f}")
